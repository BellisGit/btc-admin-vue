...
export const format: { [key: string]: any } = {
	number(value: any) {
...
function parse(method: "submit" | "bind", { value, hook: pipe, form, prop }: any) {
	...
	if (isString(pipe)) {
		...
		pipes = isArray((pipe as any)[method]) ? (pipe as any)[method] : [(pipe as any)[method]];
...
export function registerFormHook(name: string, fn: any) {
	format[name] = fn;
}
